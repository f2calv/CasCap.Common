...
public abstract class HttpEndpointCheck
{
    ...
    protected async Task<bool> IsAccessible(string requestUri, int HealthCheckExpectedHttpStatusCode = 0, CancellationToken cancellationToken = default)
    {
        ...
        {
            // Handle timeout.
            _logger.LogWarning("{serviceName}, failed: {exceptionMessage}", nameof(HttpEndpointCheck), ex.Message);
        }
        // Filter by InnerException.
        catch (TaskCanceledException ex) when (ex.InnerException is TimeoutException)
        {
            // Handle timeout.
            _logger.LogDebug("{serviceName}, Timed out: {exceptionMessage}", nameof(HttpEndpointCheck), ex.Message);
        }
        catch (TaskCanceledException ex)
        {
            // Handle cancellation.
            _logger.LogWarning("{serviceName}, Canceled: {exceptionMessage}", nameof(HttpEndpointCheck), ex.Message);
        }
        catch (Exception ex)
        {
