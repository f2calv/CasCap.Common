    ...
    IRemoteCache remoteCache, ILocalCache localCache) : IDistributedCache
{
        ...
        CommandFlags flags = CommandFlags.None) where T : class
    {
        ...
        {
            logger.LogTrace("{ClassName} unable to retrieve {Key} object type {ObjectType} from {ObjectName}",
            ...
            if (_cachingOptions.RemoteCache.IsEnabled)
            {
                ...
                {
                    logger.LogTrace("{ClassName} retrieved {Key} object type {ObjectType} from {ObjectName}",
        ...
        CommandFlags flags = CommandFlags.None) where T : class
    {
        if (_cachingOptions.RemoteCache.IsEnabled)
        {
            logger.LogTrace("{ClassName} storing {Key} object type {ObjectType} in {ObjectName}",
                nameof(DistributedCacheService), key, typeof(T), nameof(IRemoteCache));
